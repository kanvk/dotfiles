...
- **`+Lazy! restore +qa` and `+AstroUpdate +qa` can deadlock** in lazy.nvim — `+qa` queues immediately while async tasks are mid-flight, occasionally hanging nvim. Both are wrapped in `timeout --kill-after=30s 10m` (apply script for Lazy restore; `vimup` alias for AstroUpdate).
